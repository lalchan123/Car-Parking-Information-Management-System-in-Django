...
def save_vehicle(request):
    if request.method == 'POST':
        ...
        a = Customer(first_name=first_name, last_name=last_name, phone_number=phone_number, card_number=card_number, car_model=car_model, car_color=car_color, reg_date=date_time,register_name=register_name,comment=comment, cost_per_day=cost_per_day, device=device)
        a.save()
...
class GeneratePdf(ListView):
    def get(self, request, pk, *args, **kwargs):
        ...
        "data": {
            'today': 'Today', 
             'amount': 39.99,
            'customer_name': 'Lalchan Badsa',
            'order_id': 1233434,
            'location': 'Uttara, Dhaka',
            'address': 'Rupnagar, Mirpur-2',
            'email': 'lalchanbadsa12345@gmail.com',
        },
        "infos": infos,
        }
...
def create(request):
    choice = ['1', '0', 5000, 10000, 15000, 'Admin', 'Cashier']
    choice = {'choice': choice}
    if request.method == 'POST':
            first_name=request.POST['first_name']
            last_name=request.POST['last_name']
            ...
            if userType == "Cashier":
                ...
                a.save()
                messages.success(request, 'Member of Cashier was created successfully!')
